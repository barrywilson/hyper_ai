...
class ApplicationRepository:
    ...
    async def create(self, name: str, description: str | None = None) -> dict[str, Any]:
        ...
        # Insert the application row and its default configuration in a single
        # statement so both rows are committed atomically. If the configurations
        # insert fails, the applications insert is rolled back with it.
        default_config: dict = {}
        ...
        query = """
            WITH new_app AS (
                ...
                SELECT id, %s::jsonb FROM new_app
            )
            SELECT id, name, description FROM new_app
        """

        result = await self.db.execute_mutation(
            query, (app_id_str, name, description, config_json), return_id=True
        )
