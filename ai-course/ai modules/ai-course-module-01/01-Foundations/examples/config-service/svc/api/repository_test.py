...
class TestApplicationRepository:
    ...
    async def test_create_application(self, repo, mock_db_pool):
        ...
        # The application row and its default configuration are inserted in a
        # single atomic statement (CTE), so there is exactly one mutation call.
        mock_db_pool.execute_mutation.assert_called_once()
        call = mock_db_pool.execute_mutation.call_args
        query = call[0][0]
        params = call[0][1]

        assert "INSERT INTO applications" in query
        assert "INSERT INTO configurations" in query
        assert params[1] == "test-app"
