...
class TestSettings:
    ...
    def test_settings_loads_from_env_file(self, tmp_path) -> None:
        """Test that settings loads values from a .env file instead of defaults.

        Uses a temp .env file rather than a checked-in one so the test works in
        environments (CI, deployed) where .env is not present and config comes
        ...
        with patch.dict(os.environ, env_without_settings, clear=True):
            ...
            assert settings.app_version == "0.1.0"
            assert settings.debug is True  # From .env (true), default would be False
            assert settings.log_level == "INFO"
            assert settings.host == "0.0.0.0"
