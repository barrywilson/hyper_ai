...
- **Connection Management**: 
  ...
  - Async-compatible: blocking psycopg2 calls dispatched to a `ThreadPoolExecutor` (see Key Technical Decisions below)
  - Transaction support
...
- **Async-Compatible Operations**: Database calls use blocking psycopg2 dispatched to a `ThreadPoolExecutor` so they can be `await`ed from FastAPI route handlers. This is *not* end-to-end async (true async would require psycopg3 or asyncpg). Trade-off chosen for simplicity and stability.
