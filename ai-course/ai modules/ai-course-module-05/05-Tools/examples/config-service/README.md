...
- **Web UI Admin**: Web UI for centralised, remote administration
...
2. **Materialise the demo git history** (one-time, optional):
   ```sh
   make seed-history
   ```

   This unpacks `history.bundle` into a real `.git/` so skills like
   `start-work` can inspect commit history. Working-tree files are
   unchanged. Skip if you'd rather start from a clean `git init`.

3. **Install dependencies** for svc and ui
   ```sh
   make install
   ```
