...
class TestSettings:
    ...
    def test_settings_ignores_unrecognised_env_vars(self) -> None:
        """Extra environment variables that don't match a Settings field are silently ignored.

        This matches the configured policy (`extra='ignore'` in Settings.model_config).
        Adding a new envvar is non-breaking; only typos in *existing* field names matter.
        """
        env_vars = {
            "LOG_LEVEL": "INFO",
            "APP_VERSION": "1.0.0",
            ...
            "PORT": "8000",
            "DEBUG": "false",
            "EXTRA_FIELD_NOT_DEFINED": "ignored",
        }

        with patch.dict(os.environ, env_vars, clear=True):
            # Should not raise — extra envvars are dropped per `extra='ignore'` policy
            settings = Settings()

            # The recognised fields are still loaded correctly
            assert settings.log_level == "INFO"
            assert settings.port == 8000

            # The unrecognised envvar is not exposed as a field
            assert not hasattr(settings, "extra_field_not_defined")
    ...
    def test_settings_loads_from_env_file(self) -> None:
        ...
        with patch.dict(os.environ, env_without_settings, clear=True):
            ...
            assert settings.port == 8000  # From .env, same as default but proves loaded
