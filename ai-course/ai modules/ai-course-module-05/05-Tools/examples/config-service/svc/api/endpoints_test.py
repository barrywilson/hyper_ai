...
class TestApplicationAPI:
    ...
    def test_internal_error_does_not_leak_exception_message(self, mock_db_pool):
        """Unhandled internal errors return a generic 500 — no exception text leaks.

        Regression test for finding 15: the previous `except Exception as e:
        raise HTTPException(500, str(e))` wrapper leaked psycopg2 internals
        (table names, SQL fragments) to clients under DEBUG=true.

        Uses a TestClient configured to NOT propagate server exceptions
        (i.e. behave like a real client).
        """
        from api.main import unhandled_exception_handler

        # Build a TestClient that mirrors prod behavior on unhandled errors,
        # including the same global Exception handler that the production
        # `svc` app registers (returns ErrorResponse JSON for unhandled errors).
        test_app = FastAPI()
        test_app.include_router(api_router, prefix="/api/v1")
        test_app.add_exception_handler(Exception, unhandled_exception_handler)
        test_app.dependency_overrides[get_db_pool] = lambda: mock_db_pool
        try:
            prod_like_client = TestClient(test_app, raise_server_exceptions=False)

            secret_message = "psycopg2.errors.UndefinedTable: secret_internal_table"
            mock_db_pool.execute_query = AsyncMock(side_effect=RuntimeError(secret_message))

            response = prod_like_client.get("/api/v1/applications")

            assert response.status_code == 500
            body_text = response.text
            # The internal exception message must not appear anywhere in the body.
            assert secret_message not in body_text
            assert "secret_internal_table" not in body_text

            # The body should be a JSON ErrorResponse (matches the
            # `500: ErrorResponse` declared in each endpoint's OpenAPI
            # `responses` dict). The detail must be a generic string,
