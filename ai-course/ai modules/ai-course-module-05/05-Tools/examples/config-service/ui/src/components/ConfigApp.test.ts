...
describe('ConfigApp', () => {
  ...
  describe('XSS protection', () => {
    it('escapes XSS payload in application description', async () => {
      const xssPayload = '<img src=x onerror="(window as any).__pwned=1">';
      server.use(
        http.get('/api/v1/applications', () => {
          return HttpResponse.json([
            { id: '01ARZ3NDEKTSV4RRFFQ69G5FAV', name: 'Evil App', description: xssPayload },
          ]);
        })
      );

      cleanupTestComponent(component);
      component = createTestComponent(ConfigApp, 'config-app-xss');

      await new Promise(resolve => setTimeout(resolve, 100));

      const desc = component.querySelector('.app-description');
      // Description should appear as escaped text (visible to admins)
      expect(desc?.textContent).toContain('<img src=x onerror=');
      // ...but NOT be parsed as a real DOM node
      expect(desc?.innerHTML).not.toContain('<img');
      // The exfiltration check: no script ran
      expect((window as Window & typeof globalThis & { __pwned?: number }).__pwned).toBeUndefined();
    });
  });
