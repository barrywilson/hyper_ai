...
describe('ConfigDetail', () => {
  ...
  describe('Initial state', () => {
    ...
    it('should not make API call without application-id', async () => {
      // Observe behavior (network calls) rather than internal method
      // invocation. The component was created in beforeEach with no
      // application-id, so connectedCallback's loadConfiguration call
      // should have early-returned before any fetch.
      ...
      // No additional fetch should occur — application-id is unset
  ...
  describe('Attribute change handling', () => {
    ...
    it('should handle null or empty application-id by skipping API call', async () => {
      // Verify *behavior* (no network fetch) rather than just that the
      // method was invoked. loadConfiguration is invoked by
      // attributeChangedCallback for both setAttribute('') and
      // removeAttribute, but its early-return prevents any fetch.
      const fetchSpy = vi.spyOn(globalThis, 'fetch');
      const initialCalls = fetchSpy.mock.calls.length;
      ...
      // Wait a tick for any deferred load
      await new Promise(resolve => setTimeout(resolve, 0));

      // loadConfiguration exits early — no new fetch should occur
      expect(fetchSpy.mock.calls.length).toBe(initialCalls);
      fetchSpy.mockRestore();
    });
  });
