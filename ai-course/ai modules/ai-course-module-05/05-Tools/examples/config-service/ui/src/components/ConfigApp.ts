...
export class ConfigApp extends BaseComponent {
  ...
  protected render(): void {
    ...
    const createFormHtml = this.showCreateForm
      ? this.html`<create-app-form></create-app-form>`
      : '';
    ...
    this.setHTML(this.html`
      ...
      <div class="container">
        ...
        ${this.unsafeHtml(createFormHtml)}
        ${this.unsafeHtml(content)}
      </div>
    `);
  ...
  private renderContent(): string {
    ...
    const appListHtml = this.applications
      ...
      .map((app, index) => {
        ...
        return this.html`
          <li class="app-item ${this.selectedApp?.id === app.id ? 'selected' : ''}"
            ...
            ${this.unsafeHtml(descHtml)}
          </li>
        `;
      })
      .join('');

    const mainHtml = this.selectedApp
      ? this.html`
    ...
    return this.html`
      <div class="content">
        <aside class="sidebar">
          <h2>Applications</h2>
          <ul class="app-list">
            ${this.unsafeHtml(appListHtml)}
          </ul>
        </aside>
        <main class="main">
          ${this.unsafeHtml(mainHtml)}
        </main>
      </div>
    `;
  }
