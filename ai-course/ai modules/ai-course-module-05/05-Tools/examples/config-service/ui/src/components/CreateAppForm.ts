...
export class CreateAppForm extends BaseComponent {
  ...
  protected render(): void {
    ...
    const errorHtml = this.error
      ? this.html`<div class="error-message">${this.error}</div>`
      : '';
    const nameErrorHtml = this.validationErrors.name
      ? this.html`<div class="form-error">${this.validationErrors.name}</div>`
      : '';
    const descriptionErrorHtml = this.validationErrors.description
      ? this.html`<div class="form-error">${this.validationErrors.description}</div>`
      : '';

    this.setHTML(this.html`
      ${this.unsafeHtml(styles)}
      <div class="form-container ${this.loadingState === 'loading' ? 'loading' : ''}">
        <h3 class="form-title">Create New Application</h3>

        ${this.unsafeHtml(errorHtml)}
        ...
        <div class="form-group">
          ...
          ${this.unsafeHtml(descriptionErrorHtml)}
        </div>
