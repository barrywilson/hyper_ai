...
describe('ConfigClient', () => {
  ...
  describe('Config by name endpoint', () => {
    it('should return raw config dict from /config/{name}', async () => {
      // The server returns the raw config dict (endpoints.py:214 does
      // `return configuration["config"]`), not a ConfigurationResponse
      // wrapper.
      server.use(
        http.get('*/config/test-app', () => {
          return HttpResponse.json({ theme: 'dark', timeout: 30 });
        })
      );
      ...
      expect(result.success).toBe(true);
      if (result.success) {
        expect(result.data).toEqual({ theme: 'dark', timeout: 30 });
        expect((result.data as Record<string, unknown>).application_id).toBeUndefined();
      }
    });
  ...
  describe('Client configuration', () => {
    it('should use custom base URL for /api/v1 endpoints', async () => {
      // Note: /health bypasses baseUrl entirely (always at root). This test
      // covers the standard case where baseUrl is honored.
