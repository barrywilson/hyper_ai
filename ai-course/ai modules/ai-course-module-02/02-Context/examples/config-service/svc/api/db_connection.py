...
class DatabasePool:
    ...
    async def get_connection(self):
        ...
        conn = await asyncio.get_running_loop().run_in_executor(self._executor, _get_conn)

        try:
            yield conn
        finally:
            await asyncio.get_running_loop().run_in_executor(self._executor, _put_conn, conn)
    ...
    async def transaction(self):
        ...
        async with self.get_connection() as conn:
            ...
            await asyncio.get_running_loop().run_in_executor(self._executor, _begin)

            try:
                yield conn
                await asyncio.get_running_loop().run_in_executor(self._executor, _commit)
            except Exception:
                await asyncio.get_running_loop().run_in_executor(self._executor, _rollback)
                raise
            finally:
                # Reset autocommit
